...
#include <assert.h>
//TODO perror
//TODO struct with info about file
//TODO documentation
//-------------------------------------------------------------------------------------------------------------------------------------------
...
void   print_results_of_first_sorting_in_file (char** index, size_t lines_to_read, FILE* onegin_out);
int    compare_alphabet_order_reverse (const void* value_a, const void* value_b);
void   print_results_of_second_sorting_in_file (char** index, size_t lines_to_read, FILE* onegin_out);
...
void   print_original_onegin_in_file(char** index, size_t actual_lines, FILE* onegin_out);
...
int main () {
    ...
    get_size_of_file (onegin, &bytes_for_onegin);

    char* buffer = (char*)malloc(bytes_for_onegin + 1);
    check_of_memory_allocation (buffer);

    int file_onegin = open(onegin, O_RDONLY);
    check_of_opening_file (file_onegin);

    int number_of_read_info = read_file (file_onegin, &buffer, (int)bytes_for_onegin);

    size_t number_of_strings = strings_number(buffer, number_of_read_info);
    printf("Number of lines with empty strings in Onegin: %zu\n\n", number_of_strings);

    char** index = (char**)calloc(number_of_strings + 1, sizeof(char*));
    check_of_memory_allocation_index (index);

    size_t actual_lines = put_pointers_to_strings_in_index(buffer, index, number_of_read_info);

    FILE* onegin_out = fopen("onegin_out.txt", "w");
    check_of_opening_file (onegin_out);

    qsort(index, actual_lines, sizeof(char*), compare_alphabet_order);
    print_results_of_first_sorting_in_file(index, actual_lines, onegin_out);

    quick_sort(index, actual_lines, sizeof(char*), compare_alphabet_order_reverse);
    print_results_of_second_sorting_in_file(index, actual_lines, onegin_out);

    qsort(index, actual_lines, sizeof(char*), compare_from_min_to_max);
    print_original_onegin_in_file(index, actual_lines, onegin_out);

    free_alloc_plus_ptr (&index, &buffer, &bytes_for_onegin, &number_of_strings);
    close(file_onegin);
    fclose(onegin_out);
...
void quick_sort(void* data, size_t size_of_data, size_t size_of_data_element, int (*compare_func)(const void*, const void*)) {
    ...
    size_t max_arr_index = size_of_data - 1;    size_t i = 0;
...
void print_original_onegin_in_file (char** index, size_t lines_to_read, FILE* onegin_out) {
